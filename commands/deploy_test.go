...
func TestDeploy(t *testing.T) {
	...
	args = []string{"deploy", "-f", "../doc/example.yml", "--var", "TAG=1.0.2", "--var", "ENV=prod"}
	resp := Deploy(args)
...
func TestDeploy2(t *testing.T) {
	args := []string{"deploy"}
	resp := Deploy(args)
	if resp.Message != fmt.Sprintf(response.ErrorMessages["file_not_exist"], "default.yml") {
		t.Fail()
	}
}
func TestDeploy3(t *testing.T) {
	args := []string{"deploy", "-f", "../doc/example.yml", "--var", "TAG=1.0.1"}
	resp := Deploy(args)
...
func TestDeploy4(t *testing.T) {
	args := []string{"deploy", "-f", "../yaml/tests/v1/invalid.7.yml"}
	resp := Deploy(args)
	if resp.Message != response.ErrorMessages["no_time_field"] {
		t.Fail()
	}

	args = []string{"deploy", "-f", "../yaml/tests/v1/invalid.8.yml"}
	resp = Deploy(args)
	if resp.Message != response.ErrorMessages["no_hash_field"] {
		t.Fail()
	}

	args = []string{"deploy", "-f", "../yaml/tests/v1/invalid.9.yml"}
	resp = Deploy(args)
...
func TestDeploy6(t *testing.T) {
	args := []string{"deploy", "-f", "../doc/example.yml", "--var", "TAG=1.0.2", "--var", "ENV=prod", "--master", "localhost"}
	oldOut := utils.ShutUpOut()
	resp := Deploy(args)
	utils.RestoreOut(oldOut)
	if resp.Code != 0 {
